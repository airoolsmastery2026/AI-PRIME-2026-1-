...
const InlineAffiliateConnector: React.FC<{platform: ConnectablePlatform, icon: React.ReactNode}> = ({ platform, icon }) => {
    const { t } = useTranslation();
    const { addSocialAccount } = useSystem();
    ...
    const handleConnect = () => {
        ...
        setTimeout(() => {
            addSocialAccount(platform, {
                username,
                password,
                category: '', // Not needed for affiliates
                country: '',  // Not needed for affiliates
            });
            // No need to set isConnecting to false, as the component will unmount
        }, 1000);
    };
    
    const getPlatformLabels = () => {
        let usernameLabel = t('accounts.connector.affiliateIdLabel');
    ...
    return (
        <div className="hud-border p-4 flex flex-col justify-between transition-all">
            ...
            <div className="space-y-3">
                 <div>
                    <label className="block text-xs font-medium text-purple-300 mb-1">{usernameLabel}</label>
                    <input
                        type="text"
                        value={username}
                        onChange={(e) => setUsername(e.target.value)}
                        placeholder={t('accounts.connector.affiliateIdPlaceholder')}
                ...
                <div>
                    <label className="block text-xs font-medium text-purple-300 mb-1">{passwordLabel}</label>
            ...
            <button
                ...
                className="w-full mt-4 bg-purple-600/80 hover:bg-purple-600 text-white font-bold py-2 px-4 rounded-md transition-colors text-sm disabled:opacity-50"
            >
                {isConnecting ? t('accounts.connector.testingConnection') : t('accounts.connector.connectButton')}
            </button>
        </div>
    );
};
...
const AccountManagerModal: React.FC<{
    ...
    const handleSave = (data: SocialAccount | Omit<SocialAccount, 'id'>) => {
        if ('id' in data) { // Update
            ...
            addSocialAccount(platform, data);
        }
         ...
         <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50" onClick={onClose}>
                            ...
                            <div key={acc.id} className="bg-gray-800/50 p-3 rounded-md flex justify-between items-center">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    >
                                     ...
                                     <button onClick={() => removeSocialAccount(platform, acc.id)} className="p-2 bg-red-800/50 hover:bg-red-700/80 rounded-md text-white"><TrashIcon className="w-4 h-4" /></button>
                                </div>
                            </div>
                        ))
                    )}
            ...
            {linkingAccount && (
                <LinkAffiliatesModal
                    ...
                    onSave={(newLinks) => {
                        if (linkingAccount) {
                           updateSocialAccount(platform, { ...linkingAccount, linkedAffiliatePlatforms: newLinks });
                        }
                ...
                <Section title={t('accounts.sectionSocial')}>
                 ...
                 <Section title={t('accounts.sectionAffiliateGeneral')}>
                    {generalAffiliatePlatforms.map(p => {
                        const isConnected = getIsConnected(p);
                        return isConnected ? (
                            <AccountCard
                                key={p}
                                platform={p}
                            ...
                            <InlineAffiliateConnector key={p} platform={p} icon={platformIcons[p]} />
                        );
                    })}
                </Section>
